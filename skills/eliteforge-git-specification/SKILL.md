...
```bash
if [ -n "${CODEX_HOME:-}" ] && [ -d "$CODEX_HOME/skills/eliteforge-git-specification/scripts" ]; then
  ELITEFORGE_GIT_SCRIPTS="$CODEX_HOME/skills/eliteforge-git-specification/scripts"
elif [ -d "$(pwd)/skills/eliteforge-git-specification/scripts" ]; then
  ELITEFORGE_GIT_SCRIPTS="$(pwd)/skills/eliteforge-git-specification/scripts"
else
  echo "Cannot locate eliteforge-git-specification scripts. Set CODEX_HOME or run from skill repo root." >&2
  exit 1
fi
test -d "$ELITEFORGE_GIT_SCRIPTS"
```
...
5. 结构化输出  
输出内容固定为：`结论`、`命令序列`、`风险与门禁`、`提交/MR 文案`。
